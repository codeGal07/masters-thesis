...
def write_data_into_file(date, stock_name, polarity, source, current_url, file_data_path):
    with open(file_data_path, "a") as file:
        new_data_row = "{}, {}, {:.5f}, {}, {}".format(date, stock_name, polarity, source, current_url)
...
def create_file_if_not_exists(file_data_path):
    if not os.path.exists(file_data_path):
        with open(file_data_path, "w") as file:
            header = "date, stock_name, polarity, source, url\n"
            file.write(header)
